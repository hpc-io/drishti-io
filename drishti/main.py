...
insights_total = {HIGH: 0, WARN: 0, RECOMMENDATIONS: 0}
...
def convert_bytes(bytes_number):
    ...
    return f'{str(round(double_bytes, 2))} ' + tags[i]
...
def message(code, target, level, issue, recommendations=None, details=None):
    ...
    messages = [f"{color}{icon}{' [' + code + ']' if args.code else ''} {issue}"]
    ...
    if details:
        messages.extend(
            f"  {color}:left_arrow_curving_right: {detail['message']}"
    ...
    if recommendations:
        if not args.only_issues:
            ...
            for recommendation in recommendations:
                messages.append(f"    :left_arrow_curving_right: {recommendation['message']}")
...
def check_log_version(file, log_version, library_version):
    ...
    if version.parse(log_version) < version.parse('3.4.0'):
        ...
        console.print(
            Panel(
                Padding(
                    f'Converting .darshan log from {log_version} to 3.4.0: format: saving output file "{use_file}" in the current working directory.',
                    (1, 1),
                ),
                title='[orange1]WARNING',
                title_align='left',
        ...
        if not os.path.isfile(use_file):
            ret = os.system(f'darshan-convert {file} {use_file}')

            if ret != 0:
                print(f'Unable to convert .darshan file to version {library_version}')
...
def main():
    ...
    get_insights_posix(file_map, modules, report)

    #########################################################################################################################################################################

    get_insights_mpiio(file_map, modules, report)
    ...
    generate_report(NUMBER_OF_COMPUTE_NODES, codes, hints, insights_end_time, insights_start_time, job, total_files,
                    total_files_mpiio, total_files_posix, total_files_stdio)


def generate_report(NUMBER_OF_COMPUTE_NODES, codes, hints, insights_end_time, insights_start_time, job, total_files,
                    total_files_mpiio, total_files_posix, total_files_stdio):
...
def get_insights_mpiio(file_map, modules, report):
    if 'MPI-IO' in report.records:
        ...
        if df_mpiio['counters']['MPIIO_COLL_READS'].sum() == 0:
            if total_mpiio_read_operations and total_mpiio_read_operations > THRESHOLD_COLLECTIVE_OPERATIONS_ABSOLUTE:
                ...
                for index, row in df_mpiio_collective_reads.iterrows():
                    if (row['MPIIO_INDEP_READS'] + row['MPIIO_INDEP_WRITES']) and row['MPIIO_INDEP_READS'] / (
                            row['MPIIO_INDEP_READS'] + row[
                        'MPIIO_INDEP_WRITES']) > THRESHOLD_COLLECTIVE_OPERATIONS and (row['MPIIO_INDEP_READS'] + row[
                        'MPIIO_INDEP_WRITES']) > THRESHOLD_COLLECTIVE_OPERATIONS_ABSOLUTE:
                        detail.append(
                            {
                                'message': '{} ({}%) of independent reads to "{}"'.format(
                                    row['MPIIO_INDEP_READS'],
                                    row['MPIIO_INDEP_READS'] / (
                                                row['MPIIO_INDEP_READS'] + row['MPIIO_INDEP_WRITES']) * 100,
                                    file_map[int(row['id'])] if args.full_path else os.path.basename(
                                        file_map[int(row['id'])])
                                )
                            }
                        )
                ...
                recommendation = [
                    {
                        ...
                        'sample': Syntax.from_path(os.path.join(ROOT, 'snippets/mpi-io-collective-read.c'),
            ...
            issue = 'Application uses MPI-IO and read data using {} ({:.2f}%) collective operations'.format(
                ...
                df_mpiio['counters']['MPIIO_COLL_READS'].sum() / (
                            df_mpiio['counters']['MPIIO_INDEP_READS'].sum() + df_mpiio['counters'][
                        'MPIIO_COLL_READS'].sum()) * 100
            )
        ...
        if df_mpiio['counters']['MPIIO_COLL_WRITES'].sum() == 0:
            if total_mpiio_write_operations and total_mpiio_write_operations > THRESHOLD_COLLECTIVE_OPERATIONS_ABSOLUTE:
                ...
                for index, row in df_mpiio_collective_writes.iterrows():
                    if (row['MPIIO_INDEP_READS'] + row['MPIIO_INDEP_WRITES']) and row['MPIIO_INDEP_WRITES'] / (
                            row['MPIIO_INDEP_READS'] + row[
                        'MPIIO_INDEP_WRITES']) > THRESHOLD_COLLECTIVE_OPERATIONS and (row['MPIIO_INDEP_READS'] + row[
                        'MPIIO_INDEP_WRITES']) > THRESHOLD_COLLECTIVE_OPERATIONS_ABSOLUTE:
                        detail.append(
                ...
                recommendation = [
                    {
                        ...
                        'sample': Syntax.from_path(os.path.join(ROOT, 'snippets/mpi-io-collective-write.c'),
                                                   line_numbers=True, background_color='default')
                    }
                ]
            ...
            issue = 'Application uses MPI-IO and write data using {} ({:.2f}%) collective operations'.format(
                ...
                df_mpiio['counters']['MPIIO_COLL_WRITES'].sum() / (
                            df_mpiio['counters']['MPIIO_INDEP_WRITES'].sum() + df_mpiio['counters'][
                        'MPIIO_COLL_WRITES'].sum()) * 100
            )
        ...
        if df_mpiio['counters']['MPIIO_NB_READS'].sum() == 0:
                ...
                recommendation.append(
                    {
                        ...
                        'sample': Syntax.from_path(os.path.join(ROOT, 'snippets/hdf5-vol-async-read.c'),
                                                   line_numbers=True, background_color='default')
                    }
                )
            ...
            if 'MPI-IO' in modules:
                recommendation.append(
                    {
                        'message': 'Since you use MPI-IO, consider non-blocking/asynchronous I/O operations',
                        # (e.g., MPI_File_iread(), MPI_File_read_all_begin/end(), or MPI_File_read_at_all_begin/end())',
                        'sample': Syntax.from_path(os.path.join(ROOT, 'snippets/mpi-io-iread.c'), line_numbers=True,
        ...
        if df_mpiio['counters']['MPIIO_NB_WRITES'].sum() == 0:
            ...
            if 'H5F' in modules or has_hdf5_extension:
                recommendation.append(
                    {
                        'message': 'Since you use HDF5, consider using the ASYNC I/O VOL connector (https://github.com/hpc-io/vol-async)',
                        'sample': Syntax.from_path(os.path.join(ROOT, 'snippets/hdf5-vol-async-write.c'),
                                                   line_numbers=True, background_color='default')
                    }
                )
            ...
            if 'MPI-IO' in modules:
                recommendation.append(
                    {
                        'message': 'Since you use MPI-IO, consider non-blocking/asynchronous I/O operations',
                        # (e.g., MPI_File_iwrite(), MPI_File_write_all_begin/end(), or MPI_File_write_at_all_begin/end())',
                        'sample': Syntax.from_path(os.path.join(ROOT, 'snippets/mpi-io-iwrite.c'), line_numbers=True,
                                                   background_color='default')
                    }
                )
...
def get_insights_posix(file_map, modules, report):
    if 'POSIX' in report.records:
        ...
        if total_writes > total_reads and total_operations and abs(
                total_writes - total_reads) / total_operations > THRESHOLD_OPERATION_IMBALANCE:
        ...
        if total_written_size > total_read_size and abs(total_written_size - total_read_size) / (
                total_written_size + total_read_size) > THRESHOLD_OPERATION_IMBALANCE:
            issue = 'Application is write size intensive ({:.2f}% write vs. {:.2f}% read)'.format(
                total_written_size / (total_written_size + total_read_size) * 100.0,
                total_read_size / (total_written_size + total_read_size) * 100.0
            )
        ...
        if total_reads_small and total_reads_small / total_reads > THRESHOLD_SMALL_REQUESTS and total_reads_small > THRESHOLD_SMALL_REQUESTS_ABSOLUTE:
            ...
            insights_operation.append(
                message(INSIGHTS_POSIX_HIGH_SMALL_WRITE_REQUESTS_USAGE, TARGET_DEVELOPER, HIGH, issue, recommendation,
                        detail)
            )
        ...
        if total_writes_small and total_writes_small / total_writes > THRESHOLD_SMALL_REQUESTS and total_writes_small > THRESHOLD_SMALL_REQUESTS_ABSOLUTE:
            ...
            insights_operation.append(
                message(INSIGHTS_POSIX_HIGH_SMALL_READ_REQUESTS_USAGE, TARGET_DEVELOPER, HIGH, issue, recommendation,
                        detail)
            )
                ...
                recommendation.append(
                    {
                        ...
                        'sample': Syntax.from_path(os.path.join(ROOT, 'snippets/hdf5-alignment.c'), line_numbers=True,
                                                   background_color='default')
                    },
                    {
                ...
                recommendation.append(
                    {
                        ...
                        'sample': Syntax.from_path(os.path.join(ROOT, 'snippets/lustre-striping.bash'),
                                                   line_numbers=True, background_color='default')
                    }
                )
        ...
        if not shared_files.empty:
            ...
            if total_shared_reads and total_shared_reads_small / total_shared_reads > THRESHOLD_SMALL_REQUESTS and total_shared_reads_small > THRESHOLD_SMALL_REQUESTS_ABSOLUTE:
                ...
                insights_operation.append(
                    message(INSIGHTS_POSIX_HIGH_SMALL_READ_REQUESTS_SHARED_FILE_USAGE, TARGET_DEVELOPER, HIGH, issue,
            ...
            if total_shared_writes and total_shared_writes_small / total_shared_writes > THRESHOLD_SMALL_REQUESTS and total_shared_writes_small > THRESHOLD_SMALL_REQUESTS_ABSOLUTE:
                ...
                insights_operation.append(
                    message(INSIGHTS_POSIX_HIGH_SMALL_WRITE_REQUESTS_SHARED_FILE_USAGE, TARGET_DEVELOPER, HIGH, issue,
                            recommendation, detail)
                )
                ...
                recommendation.append(
                    {
                        ...
                        'sample': Syntax.from_path(os.path.join(ROOT, 'snippets/hdf5-collective-metadata.c'),
                                                   line_numbers=True, background_color='default')
                    },
                    {
                        'message': 'Since your appplication uses HDF5, try using metadata cache to defer metadata operations',
                        'sample': Syntax.from_path(os.path.join(ROOT, 'snippets/hdf5-cache.c'), line_numbers=True,
                                                   background_color='default')
                    }
                )
        ...
        for index, row in shared_files.iterrows():
            ...
            if total_transfer_size and abs(row['POSIX_SLOWEST_RANK_BYTES'] - row[
                'POSIX_FASTEST_RANK_BYTES']) / total_transfer_size > THRESHOLD_STRAGGLERS:
                stragglers_count += 1
        ...
        if stragglers_count:
            ...
            recommendation = [
                ...
                },
                {
                    ...
                    'sample': Syntax.from_path(os.path.join(ROOT, 'snippets/lustre-striping.bash'), line_numbers=True,
                                               background_color='default')
                }
            ]
        ...
        for index, row in shared_files_times.iterrows():
            ...
            if total_transfer_time and abs(row['POSIX_F_SLOWEST_RANK_TIME'] - row[
                'POSIX_F_FASTEST_RANK_TIME']) / total_transfer_time > THRESHOLD_STRAGGLERS:
                stragglers_count += 1
            ...
            recommendation = [
                {
                    ...
                    # needs to review what suggestion to give
                },
                {
                    'message': 'Consider tuning how your data is distributed in the file system by changing the stripe size and count',
                    'sample': Syntax.from_path(os.path.join(ROOT, 'snippets/lustre-striping.bash'), line_numbers=True,
        ...
        if imbalance_count:
            ...
            recommendation = [
                ...
                },
                {
                    ...
                    'sample': Syntax.from_path(os.path.join(ROOT, 'snippets/pnetcdf-hdf5-no-fill.c'), line_numbers=True,
                                               background_color='default')
                },
                {
        ...
        for index, row in aggregated.iterrows():
            if row['POSIX_BYTES_READ_max'] and abs(row['POSIX_BYTES_READ_max'] - row['POSIX_BYTES_READ_min']) / row[
                'POSIX_BYTES_READ_max'] > THRESHOLD_IMBALANCE:
                imbalance_count += 1
